...
class ReplayScreen_Impl : public ReplayScreen
{
...
public:
	ReplayScreen_Impl(IAsyncLoadableApplicationTickable* next, bool noCancel)
	{
		m_tickableToLoad = next;
		m_canCancel = !noCancel;
	}

	ReplayScreen_Impl(Game* next)
	{
		m_tickableToLoad = next;
		m_isGame = true;
	...
	virtual bool Init()
	{
		...
		m_loadingJob->OnFinished.Add(this, &ReplayScreen_Impl::OnFinished);
		if (m_lua == nullptr)
...
ReplayScreen* ReplayScreen::Create(IAsyncLoadableApplicationTickable* next, bool noCancel)
{
	return new ReplayScreen_Impl(next, noCancel);
}

ReplayScreen* ReplayScreen::Create(Game* next)
{
	return new ReplayScreen_Impl(next);
}
